...
// ── FINISH EXAM (Stabilized Retries & Dynamic DOM Targets) ──
async function finishExam(isAuto = false) {
    ...
    // Dynamic helper to ensure we always grab the active button element from the DOM
    const getSubmitBtn = () => document.getElementById('finalSubmit');

    let initialBtn = getSubmitBtn();
    if (initialBtn) { 
        initialBtn.disabled = true; 
        initialBtn.innerText = "SUBMITTING..."; 
    }
    ...
    const clearSession = () => {
        examActive = false;
        if (proctorInterval) clearInterval(proctorInterval); // Stop camera loops instantly
    ...
    const attemptSubmit = async () => {
        let activeBtn = getSubmitBtn();
        if (activeBtn) {
            activeBtn.disabled = true;
            activeBtn.innerText = "SUBMITTING...";
        }
        try {
            const response = await fetch(
                'https://gjznwgzoqpfdnxywixgv.supabase.co/functions/v1/calculate-score',
                {
                    method:  'POST',
                    headers: {
    ...
    // Isolate UI rebuilding logic to handle DOM cloning cleanly
    function setupRetryUI() {
        let currentBtn = getSubmitBtn();
        if (!currentBtn) return;

        currentBtn.disabled  = false;
        currentBtn.innerText = "🔄 RETRY SUBMISSION";
        currentBtn.style.background = "#ff8800";

        // Clone element to wipe out previous attached event triggers cleanly
        ...
        newBtn.addEventListener('click', async () => {
            ...
            if (!ok) {
                let freshBtn = getSubmitBtn();
                if (freshBtn) {
                    freshBtn.disabled  = false;
                    freshBtn.innerText = "🔄 RETRY SUBMISSION";
                }
