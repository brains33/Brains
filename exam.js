...
async function finishExam(isAuto = false) {
    if (!examActive) return;
    // Do NOT set examActive = false yet — keep timer running during submission
    // Only clear it after confirmed success or explicit retry path
    ...
    const submissionPayload = {
        ...
        questions_order: questions.map(q => ({ id: q.id })), // IDs only — no answers
    ...
    if (!success) {
        ...
        if (submitBtn) {
            ...
            // Clone to remove stacked listeners
