...
async function startCountdown() {
    ...
    const isCarryover = examType === 'carryover';
    if (!endTime) {
        const { session } = await studentFetch('get-exam-session', {
            course: activeSub,
            isCarryover
        });
    ...
    endTimeCheckInterval = setInterval(async () => {
        ...
        if (session && session.end_time) {
            const newEnd = new Date(session.end_time).getTime();
...
async function initExam() {
    ...
    let carryoverLevel = null;
    let carryoverSemester = null;
    if (examType === 'carryover') {
        carryoverLevel = sessionStorage.getItem('carryoverOriginalLevel');
        carryoverSemester = sessionStorage.getItem('carryoverOriginalSemester');
        if (carryoverLevel) queryLevel = carryoverLevel;
        if (carryoverSemester) querySemester = carryoverSemester;
    }
    ...
    try {
        ...
        if (savedProgress && savedQuestions) {
            ...
            const { questions: loadedQuestions } = await studentFetch('get-exam-questions', {
                course: activeSub,
                isCarryover: examType === 'carryover',
                originalLevel: carryoverLevel,
                originalSemester: carryoverSemester
            });
        ...
        await studentFetch('update-live-monitoring', {
            status: 'ACTIVE',
        ...
        const kickChecker = setInterval(async () => {
            ...
            try {
                const { status } = await studentFetch('check-live-status');
                if (status === 'KICKED') {
...
async function finishExam(isAuto = false) {
    ...
    // Server fetches correct answers directly from the database
...
// ── PROCTORING (updated to use studentFetch) ────────────────────────
let proctorInterval;
document.addEventListener('visibilitychange', async () => {
    if (document.hidden) {
        ...
        await studentFetch('update-live-monitoring', {
            status: 'CHEAT',
            currentSubject: sessionStorage.getItem('activeSubject') || 'Exam'
        });
    }
});
...
async function sendProctorSnapshot() {
    ...
    const student = JSON.parse(sessionStorage.getItem('student_data'));
    if (!student) return;
    const currentSubject = sessionStorage.getItem('activeSubject') || 'Exam';
    try {
        const { status: currentStatus } = await studentFetch('check-live-status');
        if (currentStatus === 'KICKED' || currentStatus === 'FORCE_SUBMIT') return;
        let newStatus = currentStatus === 'CHEAT' ? 'ACTIVE' : currentStatus;
        context.drawImage(video, 0, 0, canvas.width, canvas.height);
        const imageBase64 = canvas.toDataURL('image/jpeg', 0.3);
        await studentFetch('update-live-monitoring', {
            status: newStatus,
            lastSnapshot: imageBase64,
            currentSubject
        });
    } catch (err) {
        /* silent in prod */
