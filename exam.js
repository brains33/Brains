...
// ── SUPABASE CLIENT (direct – no edge function) ──────────────────────
...
// ── BUILD / UPDATE QUESTION PALETTE (unchanged) ──────────────────────
...
async function initExam() {
    ...
    if (!student || !activeSub) return alert("Session Error. Please log in again.");
    ...
    try {
        ...
        if (savedProgress && savedQuestions) {
            ...
            if (error || !data || data.length === 0) return alert("No questions found for " + activeSub);
        ...
        const kickChecker = setInterval(async () => {
            ...
            try {
                ...
                } else if (rec.status === 'FORCE_SUBMIT') {
                    clearInterval(kickChecker);
                    finishExam(true);
                }
            } catch(e) { /* silent */ }
...
// ── PROCTORING (using direct Supabase) ──────────────────────────────
let proctorInterval;
