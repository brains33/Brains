...
async function initExam() {
    ...
    try {
        ...
        // ── Reset any stale monitoring status before exam starts ────
        ...
        renderQuestion();
        startCountdown();
        setupProctoring(); // ← moved here so proctoring only starts when exam is live
...
async function startCountdown() {
    ...
    if (!endTime) {
        ...
        const { data, error } = await sb
            ...
            .eq('is_carryover', false)   // ← never pick up a carryover session by mistake
            .maybeSingle();
...
async function finishExam(isAuto = false) {
    if (!examActive) return;  // prevent double-trigger (e.g. timer + kick at same time)
    examActive = false;       // stop the kick checker loop immediately
...
// setupProctoring() is now called inside initExam() after status reset
// so it only starts when the exam is actually live
...
async function sendProctorSnapshot() {
    if (!examActive) return; // ← don't run if exam already ended/submitted
    ...
    try {
        ...
        // These are now handled by kickChecker — skip here to avoid double-trigger
        if (currentStatus === 'KICKED' || currentStatus === 'FORCE_SUBMIT') {
            return;
        }
