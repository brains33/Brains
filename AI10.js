...
// ── COURSE DROPDOWN (ADMIN – shows all courses from questions) ──────
async function populateCourseDropdown() {
    try {
        const { data } = await sb.from('questions').select('course');
        if (!data) return;
...
function prefillCourseFromStorage() {
    const ci = document.getElementById('courseInput');
    const lastCourse = sessionStorage.getItem('ai10_last_course');
    if (ci && lastCourse) ci.value = lastCourse;
}

// ── AUTO-PROMPT (if coming from another admin page) ─────────────────
...
async function askAI() {
    ...
    const courseCode = (courseEl?.value.trim().toUpperCase() || "GENERAL");
    if (courseEl) sessionStorage.setItem('ai10_last_course', courseCode);
    ...
    try {
        const { data, error } = await sb.functions.invoke("hyper-api", {
            body: {
                ...
                isLecturer: true,      // admin = lecturer mode
