...
function updateScheduleDepartments() {
    ...
    if (!faculty) { deptSelect.innerHTML = '<option value="">-- Select Faculty First --</option>'; return; }
...
async function loadExamSchedule() {
    ...
    if (!faculty || !dept || !level || !semester) {
        alert("Please select full group.");
        return;
    }
    const { data, error } = await sb.from('exam_schedules')
        .select('*')
        .eq('faculty', faculty)
        .eq('department', dept)
        .eq('level', level)
