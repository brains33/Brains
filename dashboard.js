...
// ── HELPER FUNCTIONS (safe DOM updates & XSS) ──────────────────────
...
window.onload = async function () {
  ...
  try {
    ...
    safeText('welcomeText', `Welcome, ${studentPackage.name.split(' ')[0]}!`);
    safeText('dspName', studentPackage.name);
    safeText('dspMatrix', studentPackage.matrix);
    safeText('sideMatrix', studentPackage.matrix);
    safeText('dspFaculty', studentPackage.faculty);
    safeText('dspDept', studentPackage.dept);
    const displayLevel = studentPackage.level ? `${studentPackage.level}L` : 'N/A';
    safeText('dspLevelSem', `${displayLevel} | ${studentPackage.semester}`);

    // Start all dashboard functions
    fetchExams();
    fetchCarryoverExams();
    syncGatekeeper();
    ...
    // Do not clear session – just show error in UI
    safeText('welcomeText', '⚠️ Could not load data. Please refresh.');
  }
};

// ── EVENT BINDING (all original listeners) ─────────────────────────
...
// ── CORE DASHBOARD FUNCTIONS (using studentFetch) ──────────────────
...
async function fetchExams() {
  ...
  try {
    ...
    listDiv.innerHTML = uniqueCourses
      .map(course => {
...
async function verifyAndStart() {
  ...
  try {
    ...
    const { session } = await studentFetch('validate-exam-token', {
      token: tokenInput,
      course: activeSub,
      isCarryover: false,
    });
...
async function syncGatekeeper() {
  ...
  try {
    ...
    if (!sessions || sessions.length === 0) {
      if (gateBadge) gateBadge.innerText = 'NO SESSION';
      if (displayToken) displayToken.innerText = '----';
    ...
    if (openSessions.length > 0) {
      ...
      if (gateBadge) {
        ...
        gateBadge.style.background = '#00ff88';
        gateBadge.style.color = '#0f5132';
      }
      if (displayToken) {
      ...
      if (gateBadge) {
        gateBadge.innerText = 'GATE CLOSED';
        gateBadge.style.background = '#ff4444';
        gateBadge.style.color = 'white';
      }
      if (displayToken) displayToken.innerText = '----';
      if (tokenInstr) tokenInstr.innerText = 'Wait for the admin to open the gate.';
    }
...
// ── RESOURCES (FULL ORIGINAL FUNCTIONS) ────────────────────────────
...
async function joinClass() {
  ...
  try {
    studentApi = new JitsiMeetExternalAPI('meet.jit.si', options);
  } catch (err) {
    alert('Video load failed.');
  }
}

// ── RESULTS (FULL PDF GENERATION) ──────────────────────────────────
        ...
        return `
          <tr style="background:${i % 2 === 0 ? '#f9f9f9' : '#fff'}">
            ...
            <td style="text-align:center;">
              <span style="display:inline-block; padding:3px 10px; border-radius:12px; font-size:0.78rem; font-weight:bold; background:${pass ? '#d1fae5' : '#fee2e2'}; color:${pass ? '#0f5132' : '#cc0000'};">
                ${pass ? 'PASS' : 'FAIL'}
              </span>
            </td>
          </tr>
  ...
  try {
    ...
    const rows = courses
      .map((c, i) => {
        ...
        const dateStr = sched.exam_date
          ? new Date(sched.exam_date).toLocaleDateString('en-GB', { weekday: 'short', day: '2-digit', month: 'short', year: 'numeric' })
          : '____________';
...
<body>
  ...
  <table>
    <thead>
      <tr><th>#</th><th>Course Code</th><th>Date</th><th>Time</th><th>Venue</th><th>Invigilator Signature</th></tr>
    </thead>
    <tbody>${rows}</tbody>
  </table>
    ...
    const rows = schedules
      .map((s, i) => {
        const dateStr = s.exam_date
          ? new Date(s.exam_date).toLocaleDateString('en-GB', { weekday: 'short', day: '2-digit', month: 'short', year: 'numeric' })
          : '—';
...
<body>
  ...
  <table>
    <thead>
      <tr><th style="width:36px;">#</th><th>Course Code</th><th>Date</th><th>Time</th><th>Venue</th><th style="width:110px;">Invigilator Signature</th></tr>
    </thead>
    <tbody>${rows}</tbody>
  </table>
  <div class="sign-section">
