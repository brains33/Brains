...
window.onload = async function() {
    ...
    try {
        ...
        if (!real) throw new Error("No student record payload returned from proxy endpoint.");
        
        const deviceId = localStorage.getItem('muujiza_device_token');
        
        // Comprehensive database property normalization path
        const rawMatrix   = real.matrix_no || real.matrix || "N/A";
        const rawDept     = real.department || real.dept || "N/A";
        const rawName     = real.name || "Student User";
        const rawFaculty  = real.faculty || "Not Specified";

        const studentPackage = {
            name: rawName,
            matrix: rawMatrix,
            faculty: rawFaculty,
            dept: String(rawDept).toUpperCase().trim(),
        ...
        // Secure string slicing fallback logic prevents runtime breakage on empty data fields
        let firstName = "Student";
        if (localData.name && typeof localData.name === 'string') {
            const cleanString = localData.name.trim();
            if (cleanString.length > 0) {
                firstName = cleanString.split(' ')[0];
            }
        }

        // DOM Element Population Checks
        if (document.getElementById('welcomeText')) {
            document.getElementById('welcomeText').innerText = `Welcome, ${firstName}!`;
        }
        if (document.getElementById('dspName')) document.getElementById('dspName').innerText = localData.name;
        if (document.getElementById('dspMatrix')) document.getElementById('dspMatrix').innerText = localData.matrix;
        if (document.getElementById('sideMatrix')) document.getElementById('sideMatrix').innerText = localData.matrix;
        if (document.getElementById('dspFaculty')) document.getElementById('dspFaculty').innerText = localData.faculty;
        if (document.getElementById('dspDept')) document.getElementById('dspDept').innerText = localData.dept;
        
        if (document.getElementById('dspLevelSem')) {
            const displayLevel = localData.level ? localData.level + 'L' : 'N/A';
            document.getElementById('dspLevelSem').innerText = `${displayLevel} | ${localData.semester || 'N/A'}`;
        }

        // Initialize Background Services & Refresh Timers
        fetchExams();
        fetchCarryoverExams();
        syncGatekeeper();
        ...
        console.error("Initialization Routine Error Block:", e);
        if (document.getElementById('welcomeText')) {
            document.getElementById('welcomeText').innerText = "⚠️ Failed to configure student card environment. Please check console metrics.";
        }
    }
};
