...
fetchCarryoverExams();  
        ...
        checkScheduledExamCardReleased();   // ✅ added
        ...
        setInterval(checkScheduledExamCardReleased, 30000);   // ✅ moved here
        syncClassroom();
...
function bindDashboardEvents() {
    ...
    // Auto-uppercase token input — so student doesn't struggle with case
...
async function verifyAndStart() {
    ...
    // ── CLIENT-SIDE TOKEN ATTEMPT RATE LIMITING ──────────────────
...
async function syncGatekeeper() {
    ...
    try {
        ...
        // Auto-expire handled server-side only — students should not update DB
        // The admin's autoCloseGate function handles this server-side
...
async function downloadExamCardPDF() {
    ...
    // 1. Fetch distinct courses from questions table
    ...
    // 2. Try to fetch manual schedule dates/times via RPC (graceful fallback if not set up)
    let scheduleMap = {};
    try {
    ...
    } catch(e) { /* No schedule available — columns will be blank */ }

    // 3. QR code payload for door scanner
    ...
    // 4. Build rows — include date/time/venue if schedule exists
...
<body>
  ...
  <div class="sign-section">
    <div class="sign-box">
      ...
      <p>Exam Officer Signature &amp; Date</p>
    </div>
    <div class="sign-box">
      <div class="sign-line"></div>
      <p>HOD / Dean Signature &amp; Date</p>
    </div>
  </div>
...
async function checkScheduledExamCardReleased() {
    if (!localData) return;
    try {
        // Uses same SECURITY DEFINER pattern as is_results_released
        // Key format matches what adminmath.js writes: DEPT_LEVEL_SEMESTER
...
async function downloadScheduledExamCardPDF() {
    ...
    // Use RPC (SECURITY DEFINER) to bypass RLS on exam_schedules
