...
async function checkScheduledExamCardReleased() {
    ...
    const key = `${faculty.trim().toUpperCase()}_${dept.trim().toUpperCase()}_${level}_${semester}`;
    try {
        const { data } = await sb.from('admin_settings').select('exam_schedule_config').eq('id', 1).maybeSingle();
        const config = data?.exam_schedule_config || {};
        const released = config[key] === true;
