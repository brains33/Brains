...
window.onload = async function() {
    ...
    try {
        // Get student info from edge function (replaces verify_student_token RPC)
        ...
        // Start core functions (all now use studentFetch)
        fetchExams();
        fetchCarryoverExams();
        syncGatekeeper();
...
// ── CORE FUNCTIONS (rewritten to use studentFetch) ─────────────────
...
async function fetchExams() {
    ...
    try {
        // Get student's finished courses (results)
        ...
        // Get exam courses (from questions) – edge function returns courses
        ...
        // Get sessions (tokens, gate status) – edge function
...
async function verifyAndStart() {
    ...
    // Client-side token attempt limiting (sessionStorage only – not critical)
    ...
    try {
        // Check if already passed (via edge function)
        ...
        // Validate token (edge function returns session if valid)
