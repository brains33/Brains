...
// ── ON LOAD INITIALIZATION ROUTINE ───────────────────────────────────
window.onload = async function() {
    ...
    try {
        const responseData = await studentFetch('get-student-info');
        if (!responseData || !responseData.student) {
            throw new Error("Invalid backend payload architecture or missing student key.");
        }
        
        const real = responseData.student;
        const deviceId = localStorage.getItem('muujiza_device_token') || 'NO_DEVICE_TOKEN';
        
        // 1. Extreme Defensive Fallbacks - Prevents runtime crashes on null/undefined values
        const rawName = String(real.name || real.student_name || "Student User").trim();
        const rawMatrix = String(real.matrix_no || real.matrix || "N/A").trim();
        const rawDept = String(real.department || real.dept || "N/A").trim();
        const rawFaculty = String(real.faculty || "Not Specified").trim();
        const rawLevel = String(real.level || "").trim();
        const rawSemester = String(real.semester || "").trim();

        const studentPackage = {
            name: rawName,
            matrix: rawMatrix,
            faculty: rawFaculty,
            dept: rawDept.toUpperCase(),
            level: rawLevel.replace(/L+$/i, '').trim(),
            semester: rawSemester,
            deviceId: deviceId
        };
        
        // Save safely to storage references
        sessionStorage.setItem('student_data', JSON.stringify(studentPackage));
        localData = studentPackage;

        // 2. Safe Name Splitting Logic
        let firstName = "Student";
        if (studentPackage.name && studentPackage.name !== "Student User") {
            const parts = studentPackage.name.split(' ');
            if (parts.length > 0 && parts[0].length > 0) {
                firstName = parts[0];
            }
        }

        // 3. Update DOM Fields Safely (Verifies elements exist before injecting text)
        if (document.getElementById('welcomeText')) {
            document.getElementById('welcomeText').innerText = `Welcome, ${firstName}!`;
        }
        if (document.getElementById('dspName')) document.getElementById('dspName').innerText = studentPackage.name;
        if (document.getElementById('dspMatrix')) document.getElementById('dspMatrix').innerText = studentPackage.matrix;
        if (document.getElementById('sideMatrix')) document.getElementById('sideMatrix').innerText = studentPackage.matrix;
        if (document.getElementById('dspFaculty')) document.getElementById('dspFaculty').innerText = studentPackage.faculty;
        if (document.getElementById('dspDept')) document.getElementById('dspDept').innerText = studentPackage.dept;
        
        if (document.getElementById('dspLevelSem')) {
            const displayLevel = studentPackage.level ? studentPackage.level + 'L' : 'N/A';
            const displaySem = studentPackage.semester || 'N/A';
            document.getElementById('dspLevelSem').innerText = `${displayLevel} | ${displaySem}`;
        }

        // 4. Fire Background Sync Framework Services
        try { fetchExams(); } catch(err) { console.error("Exam load failure:", err); }
        try { fetchCarryoverExams(); } catch(err) { console.error("Carryover load failure:", err); }
        try { syncGatekeeper(); } catch(err) { console.error("Gatekeeper system sync failure:", err); }
        try { checkResultsReleased(); } catch(err) { console.error("Results check failure:", err); }
        try { checkExamCardReleased(); } catch(err) { console.error("Exam card check failure:", err); }
        try { checkScheduledExamCardReleased(); } catch(err) { console.error("Schedules check failure:", err); }
        
        // 5. Setup Engine Interval Timers
        ...
        try { syncClassroom(); } catch(err) { console.error("Classroom sync failure:", err); }
        setInterval(syncClassroom, 60000);
        
        try { checkForLiveClass(); } catch(err) { console.error("Live video check failure:", err); }
        setInterval(checkForLiveClass, 30000);

        // CRITICAL FIX: Bind events even if background fetch methods have issues
        try { bindDashboardEvents(); } catch(err) { console.error("Event binding crash:", err); }
        try { loadMyAssignments(); } catch(err) { console.error("Assignments initialization failure:", err); }
        setInterval(loadMyAssignments, 60000);

    } catch (e) {
        console.error("CRITICAL ENVIROMENT FAULT:", e);
        if (document.getElementById('welcomeText')) {
            document.getElementById('welcomeText').innerText = "⚠️ Profile Engine Error. System running in fail-safe fallback state.";
        }
        // Force bind button listeners even during data crashes so navigation works
        try { bindDashboardEvents(); } catch(err) { console.error("Fallback binding fail:", err); }
    }
};
