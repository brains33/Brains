...
Vary question starters — never start two questions the same way.
OUTPUT NOTHING ELSE — only the raw JSON array.`,
                courseCode: course,
                mode:       "snaptest",   // ← dedicated mode, not chat
                snapCount:  Number(count)
            }
        });
        ...
        // 7. Validate the structure — snap test uses question/option1-4/answer format
        ...
        // Normalise — handle both {q,opts,ans} and {question,option1..4,answer} formats
        snapQuestions = parsed.map(item => {
            if (item.question) return item; // already in snap format
            // Convert from practice format to snap format
            return {
                question: item.q || '',
                option1:  (item.opts || [])[0] || '',
                option2:  (item.opts || [])[1] || '',
                option3:  (item.opts || [])[2] || '',
                option4:  (item.opts || [])[3] || '',
                answer:   (typeof item.ans === 'number') ? item.ans + 1 : 1
            };
        }).filter(item => item.question && item.option1);
