...
async function generatePractice() {
    ...
    try {
        ...
        // Edge function may return the array in several shapes:
        // 1. data is already a JS array  (ideal)
        // 2. data is { answer: "[{...}]" } or { answer: [{...}] }
        // 3. data is a raw string  (unlikely with functions.invoke but handle anyway)
        // Also strip markdown fences the AI sometimes adds
        let raw = data;

        // Unwrap { answer: ... } wrapper
        if (raw && typeof raw === 'object' && !Array.isArray(raw) && raw.answer !== undefined) {
            raw = raw.answer;
        }

        // If still a string, strip fences and parse
        if (typeof raw === 'string') {
            raw = raw.replace(/```json|```/gi, '').trim();
            // Extract just the JSON array portion if there's surrounding text
        ...
        currentQuiz = currentQuiz.filter(q => q.q && Array.isArray(q.opts) && q.opts.length >= 2 && typeof q.ans === 'number');
        if (currentQuiz.length === 0) {
            throw new Error("AI questions are malformed. Please try again.");
        }
