...
function applyLockout() {
    // Only write a new expiry if one isn't already running —
    // prevents resetting the clock mid-lockout on repeated calls
    ...
    if (lockoutMsg) {
        ...
        const tick = () => {
            ...
            if (!until || Date.now() >= until) {
                clearFailedAttempts();
                updateLockoutUI(); // re-enables everything
                return;
            }
        ...
        tick(); // show immediately without waiting 1 s
...
async function loginUser() {
    ...
    try {
        ...
        if (!response.ok) {
            // Server-side rate-limit hit → sync client lockout immediately
            if (result.error && result.error.toLowerCase().includes('too many')) {
                setFailedAttempts(MAX_FAILED_LOGINS);
                applyLockout();
            } else {
                // Wrong credentials — increment counter and warn user
                ...
                if (newCount >= MAX_FAILED_LOGINS) {
                    ...
                    alert(`${safeErr({ message: result.error })} — ${attemptsLeft} attempt${attemptsLeft !== 1 ? 's' : ''} remaining before lockout.`);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Restore lockout UI on page refresh / back navigation
