...
async function loginUser() {
    ...
    if (!inputName || !inputPass) {
        showLoginError('Please enter Name and Password');
        return;
    }
    ...
    try {
        ...
        if (!response.ok) {
            ...
            // Only redirect to payment if the account is explicitly "pending approval"
            if (errMsg.toLowerCase().includes('account pending approval')) {
                window.location.href = 'payment.html';
                return;
            }

            // Server‑side rate‑limit → sync client lockout
