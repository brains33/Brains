...
function showSection(name) {
    ...
    const titles = { dashboard:'Dashboard', gatekeeper:'Gatekeeper', questions:'Questions', results:'Results', release:'Release Results', students:'Students', promotion:'Session Promotion', aifeed:'AI Brain Feed', markadjust:'Mark Adjustment', carryover:'Carryover Exams'};
...
// Delegate result delete buttons (Course Performance + Master Records)
...
async function createExamSession() {
    ...
    try {
        // Get the latest normal session for this group (if any exists)
        ...
        if (existing) {
            ...
            ({ error } = await sb.from('exam_sessions')
                .insert({ faculty, department: dept, level, semester, token_code: newToken, is_active: "false", end_time: endTime.toISOString(), is_carryover: false }));
        }
        ...
        alert(`✅ Token ${newToken} → ${sanitise(dept)} | ${sanitise(level)}L | ${sanitise(semester)} Semester`);
    } catch (err) {
...
async function forceLogoutAll() {
    ...
    const confirmMsg = `🚨 WARNING: This will immediately kick out ALL students in:\n\nFaculty: ${faculty}\nDepartment: ${dept}\nLevel: ${level}L\nSemester: ${semester}\n\nTheir current answers will be LOST, and the exam gate will be CLOSED for this group. Proceed?`;
    if (!confirm(confirmMsg)) return;

    try {
        // 1. Get all matrix numbers for the targeted group
        ...
        const matrixNumbers = students.map(s => s.matrix_no);

        // 2. Kick all matching students in the live monitoring table
        const { error: kickErr } = await sb
            .from('live_monitoring')
            .update({ status: 'KICKED' })
            .in('matrix_no', matrixNumbers);

        if (kickErr) throw kickErr;

        // 3. Close the exam gate for this group – prevents re‑entry
        ...
        alert(`✅ Force logout successful!\n\n${matrixNumbers.length} student(s) have been kicked.\nThe gate is now CLOSED for ${dept} ${level}L ${semester} Semester.`);
...
async function startExam() {
    ...
    alert(`✅ Exam started for ${sanitise(dept)} ${sanitise(level)}L | ${sanitise(semester)} Semester!`);  // ← updated alert
    runVisualTimer(mins * 60, dept, level, semester);
}
function runVisualTimer(totalSeconds, dept, level, semester) {
    ...
    countdownInterval = setInterval(async () => {
        if (totalSeconds <= 0) {
            ...
            await autoCloseGate(dept, level, semester);
            return;
        }
...
async function autoCloseGate(dept, level, semester) {
...
async function deleteQuestion(id) {
    if (!confirm("Are you sure you want to delete this individual question?")) return;
    const { error } = await sb.from('questions').delete().eq('id', id);
    if (error) alert("Delete failed: " + error.message);
    else { alert("Question deleted successfully."); loadList(); }
}
async function wipeAllQuestions() {
    const searchTerm = document.getElementById("qSearch").value.trim();
    const isFiltered = searchTerm !== "";
    const message = isFiltered ? `⚠️ This will delete only the questions matching "${searchTerm}". Proceed?` : "⚠️ WARNING: This will delete EVERY question in your library. Proceed?";
    if (!confirm(message)) return;
    if (prompt("Type 'DELETE' to confirm:") !== "DELETE") return alert("Wipe cancelled.");
    try {
        let query = sb.from('questions').delete();
...
async function deleteAllResults() {
    ...
    if (!input) return;
    if (input.toUpperCase() !== "DELETE ALL") return alert("Incorrect confirmation text. Wipe cancelled.");
    const { error } = await sb.from('results').delete().neq('name', '');
    if (!error) { alert("✅ BRAINS AI Database wiped successfully."); fetchFreshData(); }
    else alert("❌ Error: " + error.message);
}
async function deleteResult(id, context) {
    if (!confirm(context === 'lecturer' ? "Remove this course record?" : "Delete from Student Master Record?")) return;
    const { error } = await sb.from('results').delete().eq('id', id);
    if (!error) fetchFreshData();
}
                ...
                <table style="width: 100%; border-collapse: collapse; font-size: 0.85em; color: white;">
                    ...
                    <tbody>${groupedByCourse[course].map(res => `
                        <tr style="border-bottom: 1px solid #222;">
                            ...
                            <td style="color: #00ff88; font-weight: bold;">${res.score}%</td>
                ...
                <table style="width:100%; color:white; font-size:0.9em; border-collapse:collapse;">
                    ...
                    <tbody>${student.exams.map(ex => `
                        <tr style="border-bottom:1px solid #222;">
                            ...
                            <td style="color:#00ff88; font-weight:bold;">${ex.score}%</td>
...
// Get matrix numbers of students currently in this carryover course
const { data: liveRows } = await sb.from('live_monitoring')
    .select('matrix_no')
    .eq('current_subject', course);
if (!liveRows || liveRows.length === 0) {
    alert("No students currently in that carryover exam.");
    return;
}
const matrixNos = liveRows.map(s => s.matrix_no);

// Kick them from live monitoring
await sb.from('live_monitoring')
    .update({ status: 'KICKED' })
...
alert(`✅ Force logout completed for ${dept} - ${course}.`);}