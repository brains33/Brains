...
async function loadBursaryStudents() {
    ...
    listDiv.innerHTML = '<p style="color:gray; padding:20px;">Loading...</p>';

    try {
        // Route through proxy so token is validated server-side
        // Direct Supabase queries with anon key bypass bursary auth
        const resp = await fetch(PROXY_URL, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'x-busa-token': token },
            body: JSON.stringify({ action: 'get-bursary-students', search: searchTerm, paidOnly: showPaidOnly })
        });
        ...
        if (!resp.ok) throw new Error(result.error || 'Failed to load students.');
        ...
        // ── UPDATE STATISTICS (from proxy response) ───────────────
        const stats = result.stats || {};
        document.getElementById('totalPaidCount').textContent = stats.totalPaidCount || 0;
        document.getElementById('totalPaidSum').textContent = '₦' + (stats.totalPaidSum || 0).toLocaleString();

        const filteredStatsDiv = document.getElementById('filteredStats');
        if (showPaidOnly && students.length > 0) {
            const eligibleSum = students.reduce((sum, cur) => sum + parseFloat(cur.payment_amount || 0), 0);
            document.getElementById('eligibleCount').textContent = students.length;
